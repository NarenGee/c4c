-- Complete the multi-role setup now that current_role column exists

-- Step 1: Update the existing current_role column with role values
UPDATE users SET "current_role" = role WHERE "current_role" IS NULL;

-- Step 2: Populate user_roles table with existing users
...
-- Step 3: Create function to switch user roles
...
BEGIN
  ...
  -- Update current_role in users table (using quoted identifier)
  UPDATE users 
...
-- Step 4: Create function to add role to existing user
...
SELECT u.id, u.email, u.role, u."current_role", 
       COUNT(ur.role) as role_count
FROM users u
LEFT JOIN user_roles ur ON ur.user_id = u.id
GROUP BY u.id, u.email, u.role, u."current_role"
LIMIT 5;
