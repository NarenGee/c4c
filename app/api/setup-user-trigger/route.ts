...
-- Also run the linked_at column migration if not done yet:
ALTER TABLE student_links ADD COLUMN IF NOT EXISTS linked_at timestamp with time zone;
UPDATE student_links SET linked_at = created_at WHERE status = 'accepted' AND linked_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_student_links_linked_at ON student_links(student_id, linked_at DESC);`

  return NextResponse.json({
    success: true,
    message: "Enhanced user profile creation trigger SQL - now handles invited users without email confirmation",
    sql: migrationSql,
    instructions: [
      "1. Go to your Supabase dashboard",
      "2. Navigate to SQL Editor", 
      "3. Paste and run the SQL above",
      "4. This trigger automatically creates user profiles when users sign up",
      "5. It handles invitation acceptance and student linking automatically",
      "6. Invited users will skip email confirmation and go straight to dashboard",
      "7. Try the invitation signup process again after running this SQL"
    ]
  })
} 