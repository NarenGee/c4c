...
export function AssignmentModal({ onAssignmentCreated }: AssignmentModalProps) {
  ...
  useEffect(() => {
    if (selectedCoach) {
      loadExistingAssignments(selectedCoach)
    } else {
  ...
  const loadExistingAssignments = async (coachId: string) => {
    if (!coachId) {
      setExistingAssignments([])
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const coachAssignments = assignments.filter((assignment: any) => 
          assignment.coach_id === coachId && assignment.is_active
        )
        setExistingAssignments(coachAssignments)
      }
    } catch (error) {
  ...
  const filteredStudents = students.filter(student => {
    ...
    const isAlreadyAssigned = existingAssignments.some(assignment => assignment.student_id === student.id)
    return matchesSearch && !isAlreadyAssigned
  })

  const alreadyAssignedStudents = students.filter(student =>
    existingAssignments.some(assignment => assignment.student_id === student.id)
  )
